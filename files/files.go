...
func Asset(name string) ([]byte, error) {
	...
	if info.IsDir() {
		return nil, fmt.Errorf("Asset %s is a Directory, not a File", name)
	}
...
func AssetCompressed(name string) ([]byte, error) {
	...
	if _, ok := file.(httpgzip.NotWorthGzipCompressing); ok {
		return nil, fmt.Errorf("Asset %s is not worth compressing", name)
	}
...
func AssetDir(name string) ([]string, error) {
	...
	if !info.IsDir() {
		return nil, fmt.Errorf("Asset %s is a File, not a Directory", name)
	}
